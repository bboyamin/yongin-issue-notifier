...
def fetch_joongang_by_date(ymd_str):
    ...
    try:
        ...
        res = requests.get(rss_url, headers=headers, timeout=8, verify=False)
    ...
    return fetch_joongang_from_naver(clean_ymd)

def fetch_joongang_from_naver(clean_ymd=None):
    ...
    url = f"https://openapi.naver.com/v1/search/news.json?query={urllib.parse.quote('중앙일보')}&display=40&sort=date"
    headers_naver = {
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            categorized = {}
            all_articles = []
            formatted_date = f"{clean_ymd[:4]}/{clean_ymd[4:6]}/{clean_ymd[6:8]}" if clean_ymd and len(clean_ymd) == 8 else datetime.now(timezone(timedelta(hours=9))).strftime("%Y/%m/%d")
