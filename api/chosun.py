...
def fetch_chosun_today():
    ...
    for section_title, rss_url in sections_list:
        try:
            res = requests.get(rss_url, headers=headers, timeout=6, verify=False)
            if res.status_code == 200 and len(res.content) > 200:
                root = ET.fromstring(res.content)
    ...
    # Tertiary: Naver OpenAPI Fallback
    return fetch_chosun_from_naver()

def fetch_chosun_from_naver():
    ...
    url = f"https://openapi.naver.com/v1/search/news.json?query={urllib.parse.quote('조선일보')}&display=30&sort=date"
    headers_naver = {
    ...
    try:
        ...
        print("Chosun Naver OpenAPI fetch error:", e)
